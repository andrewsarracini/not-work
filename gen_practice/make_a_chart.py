...
def animal_fact_generator():
    ...
    check = oi.Completion.create(engine=engine, prompt=animal_validation_string, max_tokens=1)
    ...
    response = oi.Completion.create(engine=engine, prompt=prompt, max_tokens=defualt_tokens) 
    print(response)
